	...
	for name, tc := range testCases {
		t.Run(name, func(t *testing.T) {
			...
			if tc.filterByFunc != nil {
				workflows, err = GetWorkflows(ctx, tc.mock.getMockedWorkflowServiceClient(), tc.filterByFunc)
			} else {
				workflows, err = GetWorkflows(ctx, tc.mock.getMockedWorkflowServiceClient())
			}
			if err != nil {
				if tc.err != nil {
...
func TestGetWorkflows(t *testing.T) {
	...
	workflows, err := GetWorkflows(context.Background(), client, filters...)
	if err != nil {
		t.Fatal(err)
	}
	if len(workflows) > 0 {
...
func TestGetTemplate(t *testing.T) {
	...
	client := tt.NewTemplateServiceClient(conn)
	template, err := GetTemplate(context.Background(), client, "9cda8df3-91dd-11eb-ad3b-0242ac130005")
	if err != nil {
		t.Fatal(err)
	}
	t.Log(template)
